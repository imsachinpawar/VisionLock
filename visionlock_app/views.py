...
def register_user(request):
    ...
    try:
        ...
        morse_pin = data.get("morse_pin", "").strip()
        face_encoding = data.get("face_encoding")

        # Validation
        if not username or not email or not morse_pin or not face_encoding:
        ...
        try:
            ...
            for user in UserProfile.objects.all():
                if user.face_encoding:
                    db_encoding = np.array(user.face_encoding)
...
def send_otp(request):
    try:
        ...
        cache.set(f"otp_{email}", otp, timeout=180)  # 3 minutes

        send_mail(
            subject="Your VisionLock OTP",
            message=f"Your OTP code is: {otp}",
            from_email="visionlock@app.com",
...
def verify_otp(request):
    try:
        ...
        if cached_otp is None:
            return JsonResponse({"error": "Expired or missing OTP"}, status=400)
...
def reset_pin(request):
    if request.method == "POST":
        try:
            ...
            encodings = face_recognition.face_encodings(image_np, face_locations)
            if not encodings:
                return JsonResponse({"status": "failed", "message": "No encodings"}, status=400)
