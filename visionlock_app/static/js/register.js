...
document.getElementById("start-voice").addEventListener("click", () => {
  ...
  recognition.onresult = (e) => {
    document.getElementById("voice-result").value = e.results[0][0].transcript.trim();
  };
...
document.getElementById("erase-username").addEventListener("click", () => {
  document.getElementById("voice-result").value = "";
});

document.getElementById("erase-symbol").addEventListener("click", () => {
  if (currentSymbolSequence.length > 0) {
    currentSymbolSequence.pop();
    document.getElementById("morse-display").innerText = currentSymbolSequence.join("");
  } else {
    alert("No symbol to erase.");
  }
});
  ...
  setTimeout(() => {
    ...
    .then(data => {
      if (data.username) {
        ...
        alert("✅ Face matched: " + data.username);
      } else if (data.face_encoding) {
        faceEncoding = data.face_encoding;
        alert("✅ Face scanned. No match. Will register this face.");
      } else {
